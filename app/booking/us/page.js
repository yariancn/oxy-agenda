...
export default function BookingUS() {
  ...
  const activeClinic = 'Shenandoah';
  const [selectedService, setSelectedService] = useState(null);
  const [selectedDate, setSelectedDate] = useState('');
  const [selectedTime, setSelectedTime] = useState('');
  const [formData, setFormData] = useState({ name: '', phone: '', email: '', lada: '+1', notes: '' });
    ...
    const fetchPublicData = async () => {
      ...
      try {
        const [resSrv, resApp, resBlock, resConf] = await Promise.all([
          supabaseShenandoah.from('services').select('*').eq('is_active', true),
          supabaseShenandoah.from('appointments').select('equipment, full_date, time, duration, buffer, check_in_status').neq('check_in_status', 'Cancelado'),
          supabaseShenandoah.from('blocked_slots').select('*'),
          supabaseShenandoah.from('company_config').select('*').eq('clinic', activeClinic).maybeSingle()
        ]);
  ...
  const getAvailableSlots = () => {
    ...
    for (let m = startMins; m < endMins; m += interval) {
      ...
      if (checkAvailability(timeStr, selectedService.name, selectedDate, selectedService.duration, selectedService.buffer)) {
        slots.push(timeStr);
      }
    }
    return slots;
  ...
  const dateOptions = useMemo(() => {
    const dates = [];
    const start = new Date(new Date().toLocaleString("en-US", { timeZone: 'America/Chicago' }));
    for (let i = 0; i < 14; i++) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (cleanPhone.length !== 10) {
      alert("⚠️ Error: The phone number must be exactly 10 digits. Please check.");
      return;
    }
    ...
    try {
      ...
      const payload = { 
        ...
        buffer: Number(selectedService.buffer) || 0, 
        ...
        is_new_patient: true,
        notes: `Portal TX. Prospecto Web. ${formData.notes}`
      };
      const { error } = await supabaseShenandoah.from('appointments').insert([payload]);
    ...
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900 flex flex-col">
      <header className="bg-slate-900 text-white p-4 flex items-center justify-between shadow-lg">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-sm font-black uppercase tracking-widest">Regenoxy Texas</h1>
            <p className="text-[9px] text-blue-400 font-bold uppercase">Reservaciones Shenandoah</p>
          </div>
        </div>
              ...
              <div className="space-y-6">
                ...
                {selectedDate && (
                  <div className="grid grid-cols-3 md:grid-cols-4 gap-3">
                    ...
                    {getAvailableSlots().length === 0 && <p className="col-span-full text-center text-slate-400 font-bold uppercase py-4">No slots available for this day.</p>}
                  </div>
                )}
              </div>
            )}
            {step === 3 && (
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="flex gap-2">
                  <input required placeholder="+1" value={formData.lada} onChange={e => setFormData({...formData, lada: e.target.value})} className="w-20 p-4 border-2 bg-slate-50 rounded-xl font-black text-center outline-none focus:border-blue-500" />
                  <input required type="tel" maxLength="10" placeholder="Number (10 digits)" value={formData.phone} onChange={e => setFormData({...formData, phone: e.target.value.replace(/\D/g, '')})} className="flex-1 p-4 border-2 rounded-xl font-bold outline-none focus:border-blue-500" />
                </div>
                ...
                <p className="text-[10px] text-center font-bold text-slate-400 uppercase">10 digits are required to confirm.</p>
              </form>
            )}
            {step === 4 && (
              <div className="text-center py-10">
                ...
                <p className="text-sm font-bold text-slate-500 uppercase mt-4">Te esperamos en nuestra sucursal de Shenandoah, Texas.</p>
