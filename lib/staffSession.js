...
/** Sliding window: each successful API renews; idle tablets stay signed in up to 24h. */
...
export function readStaffSessionFromRequest(request) {
  ...
  const user = verifyStaffSessionToken(decodeURIComponent(match[1]));
  return normalizeStaffSessionUser(user);
}
