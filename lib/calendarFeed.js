...
import { isClinicOpenOnDate, resolveCompanyConfigForDate } from './clinicWeeklySchedule.js';
...
/** Abre el diálogo de Google Calendar para suscribirse por URL. */
export function buildGoogleCalendarSubscribeUrl(feedUrl) {
  if (!feedUrl) return '';
  return `https://calendar.google.com/calendar/r/settings/addbyurl?url=${encodeURIComponent(feedUrl)}`;
}
...
export function buildAvailabilitySlotsForRange({
  ...
  for (const date of iterateIsoDates(startDate, toDate)) {
    if (!isClinicOpenOnDate(companyConfig, date)) continue;

    const dayConfig = resolveCompanyConfigForDate(companyConfig, date);
...
/** Firma del contenido del feed para ETag (Google revalida cuando cambia). */
export function computeFeedRevision({
  appointments = [],
  blockedSlots = [],
  services = [],
  promoterCode = '',
  ...
  for (const row of blockedSlots) {
    parts.push(row.id, row.date, row.end_date || row.date, row.start_time, row.end_time, row.equipment || '', row.is_global ? '1' : '0');
  }
  for (const row of services) {
    parts.push(row.name, row.is_active === false ? '0' : '1', row.duration, row.buffer);
  }
  let hash = 0;
  ...
  return `oxy-${Math.abs(hash).toString(36)}`;
}
