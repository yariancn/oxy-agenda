...
export function loadCustomerEmailIndex(csvPath) {
  const customers = loadSetmoreCustomers(csvPath);
  const map = new Map();
  for (const row of customers) {
    const phone = digitsOnly(row.Phone).slice(-10);
    if (phone.length === 10 && row.Email) map.set(phone, row.Email);
  }
  return map;
}

/** Full Setmore customer export (Name, Phone, Email) — includes clients without 2026 appointments. */
export function loadSetmoreCustomers(csvPath) {
  ...
  for (const line of lines.slice(1)) {
    ...
    seen.add(phone);
    out.push({ Name: name, Phone: phone, Email: email });
  }
  return out;
}
