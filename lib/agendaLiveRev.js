...
async function mirrorLivePing(supabase, clinicId, rev) {
  // Optional public ping table for Realtime (no secrets). Ignore if missing.
  const { error } = await supabase
    .from('agenda_live_ping')
    .upsert({ clinic: clinicId, agenda_rev: rev, updated_at: new Date().toISOString() }, { onConflict: 'clinic' });
  if (error && !/agenda_live_ping|relation|schema cache/i.test(error.message || '')) {
...
/**
 * O(1) bump so other open sessions notice the change on the next ping / Realtime event.
...
export async function bumpAgendaLiveRev(supabase, clinicName) {
  ...
  await mirrorLivePing(supabase, clinicId, next).catch(() => null);
