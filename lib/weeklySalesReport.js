...
import { filterExpensesByDateRange, sumExpenses } from './pettyCash.js';
...
/** Mes calendario anterior completo (CDMX). Útil el día 1 a las 6 AM = tras el cierre del último día. */
export function getPreviousMonthRange(timeZone = REPORT_TIMEZONE, referenceDate = new Date()) {
  const todayIso = todayIsoInTimezone(timeZone, referenceDate);
  ...
  return { startDate, endDate, year: prevYear, month: prevMonth };
}

export function isFirstDayOfMonthInTimezone(timeZone = REPORT_TIMEZONE, date = new Date()) {
  const iso = todayIsoInTimezone(timeZone, date);
...
export function summarizeSalesRows(rows = []) {
  ...
  for (const tx of rows) {
    ...
    total += price;
    txCount += 1;
    const allocation = allocateSaleByMethod(tx);
...
export async function loadGdlSalesData({ startDate, endDate } = {}) {
  ...
  const [patientRows, groupRows, expenseRows] = await Promise.all([
    fetchAllRows(supabase, 'patients'),
    fetchAllRows(supabase, 'session_groups'),
    supabase
      .from('petty_cash_expenses')
      .select('*')
      .eq('clinic', CLINIC_OXYGENDGL)
      .then((res) => {
        if (res.error) {
          if (/petty_cash_expenses|schema cache|does not exist/i.test(res.error.message || '')) {
            return [];
          }
  ...
  const expenses = filterExpensesByDateRange(expenseRows || [], startDate, endDate);
  return { patients, sessionGroups, rows, expenses };
}
...
function buildPdfBuffer({
  ...
  return new Promise((resolve, reject) => {
    ...
    const cashIn = summary.byMethod.cash || 0;
    const cashNet = Math.round((cashIn - expensesTotal) * 100) / 100;
    ...
    doc.fontSize(14).font('Helvetica-Bold').text('Caja chica (efectivo)');
    doc.moveDown(0.4);
    doc.fontSize(11).font('Helvetica');
    doc.text(`Ingresos en efectivo: ${formatMoney(cashIn, currency)}`);
    doc.text(`Gastos caja chica: ${formatMoney(expensesTotal, currency)}`);
    doc.font('Helvetica-Bold').text(`Neto efectivo: ${formatMoney(cashNet, currency)}`);
    doc.font('Helvetica');
    doc.moveDown(0.5);
    if (expenses.length) {
      doc.fontSize(10).font('Helvetica-Bold').text('Detalle de gastos');
      doc.moveDown(0.3);
      doc.font('Helvetica').fontSize(9);
      for (const exp of expenses.slice(0, 60)) {
        const when = String(exp.created_at || '').slice(0, 10);
        const who = String(exp.created_by || '').trim();
        doc.text(
          `${when}  −${formatMoney(exp.amount, currency)}  ${pdfText(exp.reason, 48)}${who ? ` · ${pdfText(who, 20)}` : ''}`,
        );
      }
      if (expenses.length > 60) {
        doc.text(`… +${expenses.length - 60} más`);
      }
    } else {
      doc.fontSize(10).text('Sin gastos de caja chica en este periodo.');
    }
    doc.moveDown(1);
    ...
    for (const tx of paidRows) {
      ...
      const cells = [
        ...
        pdfText(formatSalePaymentMethod(tx, 'es') || tx.paymentMethod, 16),
...
export async function runGdlSalesReportEmail({
  startDate,
  endDate,
  kind = 'weekly',
} = {}) {
  ...
  const range = startDate && endDate
    ? { startDate, endDate }
    : (isMonthly ? getPreviousMonthRange() : getPreviousWeekRange());
  ...
  const { rows, expenses = [] } = await loadGdlSalesData({ startDate: start, endDate: end });
  ...
  const prefix = isMonthly ? 'ingresos_gdl_mensual' : 'ingresos_gdl';
  const filename = `${prefix}_${start}_${end}.pdf`;
  const subject = isMonthly
    ? `Reporte mensual de ingresos GDL (${start} — ${end})`
    : `Reporte semanal de ingresos GDL (${start} — ${end})`;
  const periodLabel = isMonthly ? 'mensual' : 'semanal';
  const html = `
    <p>Hola,</p>
    <p>Adjunto el reporte ${periodLabel} de ingresos de <strong>${clinicLabel}</strong>.</p>
    ...
    <ul>
      <li>Efectivo: ${formatMoney(cashIn, currency)}</li>
    ...
    <p><strong>Caja chica:</strong> gastos ${formatMoney(expensesTotal, currency)} · neto efectivo ${formatMoney(cashNet, currency)}</p>
...
export async function runWeeklySalesReportGdl({ startDate, endDate } = {}) {
  return runGdlSalesReportEmail({ startDate, endDate, kind: 'weekly' });
}

export async function runMonthlySalesReportGdl({ startDate, endDate } = {}) {
  return runGdlSalesReportEmail({ startDate, endDate, kind: 'monthly' });
}
