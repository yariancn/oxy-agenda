...
export async function saveCompanyConfigRow(supabase, { id, clinic, payload }) {
  if (id) {
    ...
    if (isMissingColumnError(error)) {
      let partial = omitKeys(payload, [...SCHEDULE_KEYS, ...NOTIFY_KEYS, ...EMAIL_TEMPLATE_KEYS]);
      ({ error } = await supabase.from('company_config').update(partial).eq('id', id));
      if (!error) {
        return {
          error: null,
          warning: 'Se guardó parcialmente. Ejecuta scripts/supabase-company-config.sql y scripts/supabase-email-templates.sql en Supabase.',
        };
      }
    }
