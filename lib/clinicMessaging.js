...
export { isMexicoSmsConfigured, mexicoSmsProviderLabel };
...
export function isTwilioConfigured() {
  ...
  const phone = Boolean(process.env.TWILIO_PHONE_NUMBER);
  const messagingService = Boolean(getTwilioMessagingServiceSid());
  return sid && token && (phone || messagingService);
}
...
export async function sendTwilioSms({ to, body }) {
  ...
  if (!twilioSid || !twilioToken || (!twilioPhone && !messagingServiceSid)) {
    return { ok: false, error: 'Missing Twilio credentials', channel: 'sms' };
  }

  const normalize = (n) => String(n || '').replace(/\D/g, '');
  if (twilioPhone && normalize(to) === normalize(twilioPhone)) {
    return {
      ok: false,
  ...
  if (res.ok) return { ok: true, channel: 'sms', via: messagingServiceSid ? 'messaging_service' : 'phone' };
...
export async function sendPatientTextMessage({
  ...
  if (isMexicoSmsConfigured()) {
    return sendMexicoSms({ to: phone, body: smsBody, clinicName });
  }
...
export async function sendStaffTextMessages({ phones, smsBody, clinicName, whatsappBodyParams, locale = 'es' }) {
  ...
  if (isMexicoSmsConfigured()) {
    ...
    let provider;
    for (const phone of phones) {
      const result = await sendMexicoSms({ to: phone, body: smsBody, clinicName });
      if (result.ok && result.provider) provider = result.provider;
      results.push({ to: phone, ok: result.ok });
    }
    const sent = results.filter((r) => r.ok).length;
    return { ok: sent > 0, sent, total: results.length, channel: 'sms', provider };
  }
...
export function textChannelLabel(clinicName, locale = 'es') {
  if (isUsClinic(clinicName)) return 'SMS';
  if (isMexicoSmsConfigured()) return 'SMS';
  return 'WhatsApp';
}

export function mexicoTextChannelHint(locale = 'es') {
  if (isMexicoSmsConfigured()) {
    return `${mexicoSmsProviderLabel(locale)} — ${locale === 'en' ? 'Mexico' : 'México'}`;
  }
  return locale === 'en' ? 'WhatsApp (Mexico)' : 'WhatsApp (México)';
}
