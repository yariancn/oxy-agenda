...
export async function submitPublicBooking({
  ...
  const patientResult = await ensurePatient(supabase, {
    ...
    prefers_email: true,
    prefers_sms: clinicName === 'Shenandoah' ? formData.smsConsent === true : true,
  });
  ...
  const payload = {
    ...
    notes: buildPortalBookingNotes({
      ...
      locale,
      smsConsent: clinicName === 'Shenandoah' ? formData.smsConsent === true : undefined,
    }),
  ...
  const { error } = await insertStaffAppointment(supabase, payload, { skipSlotCheck: true });
  if (error) {
    return { error };
  }
