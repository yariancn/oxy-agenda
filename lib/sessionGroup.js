import { applyPurchaseSessions, reversePurchaseSessions, sumWalletBalance } from './sessionWallet.js';
...
export function normalizeGroup(row) {
  ...
  return {
    ...
    packageHistory: row.package_history || [],
...
export function migrateTitularWalletsToGroup(titularWallets = {}, packageHistory = []) {
  ...
  for (const [key, qty] of Object.entries(titularWallets || {})) {
...
export async function createSessionGroup(supabase, { name, titularPatient, patients }) {
  ...
  const wallets = migrateTitularWalletsToGroup(titularPatient.wallets, titularPatient.packageHistory);
...
export function reverseGroupPurchase(group, tx) {
  ...
  const reversed = reversePurchaseSessions(group.wallets, group.adeudo, {
    ...tx,
    equipment: walletKey,
  });
  return {
