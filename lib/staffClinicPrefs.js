import { ALL_CLINICS, CLINIC_OXYGENDGL, LEGACY_GDL_CLINIC, normalizeClinicId, uniqueCanonicalClinicIds } from './clinicRegistry.js';
...
function migrateLegacyStoredClinic(value) {
  if (value === LEGACY_GDL_CLINIC) return CLINIC_OXYGENDGL;
  if (ALL_CLINICS.includes(value)) return value;
  return null;
}
...
export function saveStaffActiveClinic(clinic) {
  ...
  if (!ALL_CLINICS.includes(id)) return;
  try {
