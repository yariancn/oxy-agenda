...
async function sendReminderChannels({
  ...
  const sendEmail = companyConfig.notify_channel_email !== false;
  const sendSms = companyConfig.notify_channel_sms !== false;
...
export async function runAppointmentReminderCron({
  ...
  if (companyConfig.notify_channel_sms === false && companyConfig.notify_channel_email === false) {
