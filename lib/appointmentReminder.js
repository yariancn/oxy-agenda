...
export async function runAppointmentReminderCron({
  ...
  // email no existe en appointments (vive en patients); SMS no lo necesita.
  const { data: rows, error } = await supabase
    .from('appointments')
    .select('id, patient, phone, time, full_date, equipment, duration, buffer, prefers_email, prefers_sms, check_in_status, reminder_sent_at')
