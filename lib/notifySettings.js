 ...
 * - First visit: always email + SMS (clinic-wide kills still apply)
 * - Email: on by default for every other notice
 * - SMS reminders: on by default (prefers_sms_reminder !== false)
 * - SMS scheduling/changes (booking/reschedule/cancel): opt-in (prefers_sms === true)
...
export function resolveNotifyChannelsForPatient(
  ...
  if (type === 'first') {
    return {
      sendEmail: clinicDefault.sendEmail,
      sendSms: clinicDefault.sendSms,
      clinicDefault,
    };
  }

  const wantsEmail = patientPrefersEmail(prefers_email);
  const wantsSms = type === 'reminder'
    ? patientPrefersSmsReminder(prefers_sms_reminder)
    : patientPrefersSmsScheduling(prefers_sms);

  return {
    sendEmail: clinicDefault.sendEmail && wantsEmail,
