...
export function resolveSessionInstructions(
  ...
  // Upgrade from legacy one-liner defaults to the full page-aligned notes.
  const LEGACY_ES = 'Evitar comidas pesadas 2 horas antes de la sesión.';
  const LEGACY_EN = 'Avoid heavy meals 2 hours before your session.';
  if (locale === 'en') {
    if (!isMeaningfulSessionInstruction(stored) || stored === LEGACY_EN || stored === LEGACY_ES || stored === ES_SESSION_DEFAULT) {
      return EN_SESSION_DEFAULT;
    }
    return stored;
  }
  if (!isMeaningfulSessionInstruction(stored) || stored === LEGACY_ES || stored === LEGACY_EN) {
    return ES_SESSION_DEFAULT;
  }
  return stored;
}
...
/** Texto completo para correo de primera sesión GDL (alineado a oxygengdl.com/indicaciones-para-sesiones/). */
export const ES_SESSION_DEFAULT = `Para garantizar tu seguridad y la efectividad de tu tratamiento, por favor lee estas indicaciones antes de tu sesión de Cámara Hiperbárica:

🚫 Restricciones importantes
• Sustancias: no consumir alcohol ni drogas 8 horas antes.
• Tabaco: no fumar 24 h antes ni después de la sesión.
• Vuelos: no volar 24 h antes ni después de la sesión.
• Salud: no ingresar con fiebre o infección respiratoria. Pacientes con historial de neumotórax, barotrauma o implantes electrónicos no compatibles no podrán ingresar.

👕 Vestimenta e higiene
• Venir bien aseado, sin cremas, perfumes ni lociones.
• Usar ropa cómoda (de preferencia pants).
• Uñas limpias, cortas y sin esmalte fresco.
• Prohibido: joyas, relojes, llaves, clips y dispositivos electrónicos.
• Ojos: usar anteojos si los necesitas; retirar lentes de contacto.

⏰ Sobre tu cita
• Primera vez: llegar 30 minutos antes para valoración e inducción.
• Identificación: presentar identificación vigente en tu primera sesión.
• Puntualidad: si llegas tarde, se descontará el tiempo de tu sesión.
• Cancelaciones: cambios o cancelaciones con 24 h de anticipación, o la sesión se dará por tomada.

📍 Ubicación
General San Martín #420 (Esq. José Guadalupe Montenegro), Guadalajara.

📞 Contacto
Cualquier programación o dudas: 33 2166 4083`;

export const EN_SESSION_DEFAULT = `To keep your session safe and effective, please read these instructions before your hyperbaric oxygen session:

🚫 Important restrictions
• Substances: no alcohol or drugs for 8 hours before.
• Tobacco: do not smoke for 24 hours before or after the session.
• Flights: do not fly for 24 hours before or after the session.
• Health: do not attend with fever or a respiratory infection. Patients with a history of pneumothorax, barotrauma, or incompatible electronic implants cannot enter.

👕 Clothing & hygiene
• Arrive clean, without creams, perfumes, or lotions.
• Wear comfortable clothes (pants preferred).
• Clean, short nails without fresh polish.
• Not allowed: jewelry, watches, keys, clips, or electronic devices.
• Eyes: wear glasses if needed; remove contact lenses.

⏰ About your appointment
• First visit: arrive 30 minutes early for assessment and orientation.
• ID: bring a valid ID for your first session.
• Punctuality: if you arrive late, time will be deducted from your session.
• Cancellations: changes or cancellations with 24 hours’ notice, or the session will count as taken.

📍 Location
General San Martín #420 (corner José Guadalupe Montenegro), Guadalajara.

📞 Contact
Scheduling or questions: 33 2166 4083`;
 ...
 * Clinic defaults keep email + SMS channels enabled; patient prefs refine SMS.
 */
export function resolveNotifyChannels(companyConfig = {}, notifyType = 'booking') {
  ...
  // Clinic keeps SMS available for all event types unless explicitly disabled.
...
/** Email stays on unless the patient explicitly opts out (`false`). */
export function patientPrefersEmail(prefers_email) {
  return prefers_email !== false;
}

/**
 * SMS for booking / reschedule / cancel: opt-in (`true`).
 * Kept for backward compatibility with prefers_sms column.
 */
export function patientPrefersSmsScheduling(prefers_sms) {
  return prefers_sms === true;
}

/** @deprecated use patientPrefersSmsScheduling */
export function patientPrefersSms(prefers_sms) {
  return patientPrefersSmsScheduling(prefers_sms);
}

/** SMS for reminders: on by default (opt-out with prefers_sms_reminder === false). */
export function patientPrefersSmsReminder(prefers_sms_reminder) {
  return prefers_sms_reminder !== false;
}

/**
 * Patient notification prefs:
 * - Email: always on for first visit, booking, reschedule, cancel, and reminder (clinic-wide email kill still applies)
 * - SMS reminders: on by default (prefers_sms_reminder !== false)
 * - SMS scheduling/changes (booking/reschedule/cancel): only if enabled (prefers_sms === true)
 * - First-visit SMS: clinic SMS channel, unless the patient fully opted out (STOP)
 * Clinic-wide "Permitir correo/SMS" remain hard kill-switches.
 */
export function resolveNotifyChannelsForPatient(
  companyConfig = {},
  notifyType = 'booking',
  { prefers_email, prefers_sms, prefers_sms_reminder } = {},
) {
  const type = normalizeNotifyType(notifyType);
  const clinicDefault = resolveNotifyChannels(companyConfig, type);
  const fullyOptedOutSms = prefers_sms === false && prefers_sms_reminder === false;
  const clinicEmailOn = companyConfig.notify_channel_email !== false;

  if (type === 'first') {
    return {
      sendEmail: clinicEmailOn,
      sendSms: clinicDefault.sendSms && !fullyOptedOutSms,
      clinicDefault,
    };
  }

  const wantsSms = type === 'reminder'
    ? patientPrefersSmsReminder(prefers_sms_reminder)
    : patientPrefersSmsScheduling(prefers_sms);

  return {
    sendEmail: clinicEmailOn,
    sendSms: clinicDefault.sendSms && wantsSms,
    clinicDefault,
  };
}

/** Mexico: 12h before for every patient. Houston: Admin value, fallback 24. */
export function resolveReminderHours(companyConfig = {}, clinicName = '') {
  const n = Number(companyConfig.reminder_hours);
  if (isShenandoah(clinicName)) return n > 0 ? n : 24;
...
export function getAutoNotifyBlockReason(companyConfig = {}, notifyType, locale = 'es') {
  ...
  if (typeKey) {
    // All auto types stay on unless explicitly false (clinic default: everything enabled).
    const disabled = companyConfig[typeKey] === false;
    if (disabled) {
      const labels = es
