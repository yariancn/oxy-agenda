 ...
 * Non-first defaults: email on, SMS off (Admin must explicitly enable SMS per event).
...
export function resolveNotifyChannels(companyConfig = {}, notifyType = 'booking') {
  ...
  // SMS is opt-in at clinic level for non-first events (email remains default).
  const useSms = companyConfig[`notify_use_sms_${type}`] === true;
  return {
...
/** SMS is opt-in: only when the patient checkbox is explicitly checked (`true`). */
export function patientPrefersSms(prefers_sms) {
  return prefers_sms === true;
}

/**
 * Patient notification prefs for non-first notices:
 * - Email on by default (opt-out with prefers_email === false)
 * - SMS off by default (opt-in with prefers_sms === true) — covers booking/reminder/etc.
...
export function resolveNotifyChannelsForPatient(
  ...
  const wantsSms = patientPrefersSms(prefers_sms);
  return {
