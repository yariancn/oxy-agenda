 ...
 * Example: booking email-only, reminder SMS-only.
 */
...
export function getAutoNotifyBlockReason(companyConfig = {}, notifyType, locale = 'es') {
  ...
  const { sendEmail, sendSms } = resolveNotifyChannels(companyConfig, type);
  if (!sendEmail && !sendSms) {
    return es
      ? 'Este aviso no tiene correo ni SMS activos en Admin → Mensajes.'
      : 'This notice has neither email nor SMS enabled in Admin → Messages.';
  }
