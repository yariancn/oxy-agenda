...
export async function insertStaffAppointment(supabase, payload, { skipSlotCheck = false } = {}) {
  if (!skipSlotCheck) {
    const slotCheck = await validateAppointmentSlot(supabase, payload);
    if (!slotCheck.ok) {
      if (slotCheck.code === SLOT_UNAVAILABLE) {
        return { data: null, error: new Error(SLOT_UNAVAILABLE) };
      }
      return { data: null, error: slotCheck.error || new Error('SLOT_CHECK_FAILED') };
    }
  }
  return saveWithSchemaFallback(supabase, 'insert', { payload });
}

export async function updateStaffAppointment(supabase, id, payload, { skipSlotCheck = false } = {}) {
  if (!skipSlotCheck) {
    const slotCheck = await validateAppointmentSlot(supabase, payload, { excludeAppointmentId: id });
    if (!slotCheck.ok) {
      if (slotCheck.code === SLOT_UNAVAILABLE) {
        return { data: null, error: new Error(SLOT_UNAVAILABLE) };
      }
...
/** Notas del día + contacto en cita (sin validar slot). */
export async function updateAppointmentNotesAndContact(supabase, id, payload) {
  return saveWithSchemaFallback(supabase, 'update', { id, payload });
}
