...
import { cashAmountFromSale } from './paymentMethod.js';
import {
  computeCashDrawerTotals,
  filterExpensesSince,
  getPettyCashFloat,
  sumExpenses,
} from './pettyCash.js';
...
/**
 * Cash sales + petty cash expenses after last retiro (exclusive).
 * expectedInDrawer = float + sales − expenses
 * withdrawAmount = sales − expenses (float stays in drawer)
 */
export function collectCashSalesSinceCut({
  ...
  const cash = all
    .map((tx) => {
      const cashPortion = cashAmountFromSale(tx);
      if (cashPortion <= 0) return null;
      const ms = saleTxCreatedMs(tx);
      if (sinceMs > 0 && ms > 0 && ms <= sinceMs) return null;
      return { ...tx, cashPortion };
    })
    .filter(Boolean);
  ...
  const cashSalesTotal = Math.round(
    cash.reduce((sum, tx) => sum + (Number(tx.cashPortion) || 0), 0) * 100,
  ) / 100;
  const periodExpenses = filterExpensesSince(expenses, sinceMs)
    .sort((a, b) => expenseCreatedMsSafe(a) - expenseCreatedMsSafe(b));
  const expensesTotal = sumExpenses(periodExpenses);
  const floatAmount = getPettyCashFloat(clinic);
  const totals = computeCashDrawerTotals({
    cashSalesTotal,
    expensesTotal,
    floatAmount,
  });
  ...
  return {
    ...
    expectedCash: totals.expectedInDrawer,
    expectedInDrawer: totals.expectedInDrawer,
    withdrawAmount: totals.withdrawAmount,
    cashSalesTotal: totals.cashSalesTotal,
    expensesTotal: totals.expensesTotal,
    floatAmount: totals.floatAmount,
    ticketCount: cash.length,
    expenseCount: periodExpenses.length,
  };
}

function expenseCreatedMsSafe(row) {
  const t = Date.parse(row?.created_at || row?.createdAt || '');
  return Number.isNaN(t) ? 0 : t;
}
...
export function buildCashCutRecord({
  ...
  const expected = Math.round((Number(expectedInDrawer ?? expectedCash) || 0) * 100) / 100;
  ...
  const salesTotal = cashSalesTotal != null
    ? Math.round((Number(cashSalesTotal) || 0) * 100) / 100
    : Math.round(sales.reduce((s, tx) => s + (Number(tx.price) || 0), 0) * 100) / 100;
  const expTotal = expensesTotal != null
    ? Math.round((Number(expensesTotal) || 0) * 100) / 100
    : Math.round(expenses.reduce((s, e) => s + (Number(e.amount) || 0), 0) * 100) / 100;
  const toWithdraw = withdrawAmount != null
    ? Math.round((Number(withdrawAmount) || 0) * 100) / 100
    : Math.round((salesTotal - expTotal) * 100) / 100;
  return {
    id: Date.now(),
    closedAt,
    ...
    receivedBy: recibe,
    clinic,
    floatAmount: Math.round((Number(floatAmount) || 0) * 100) / 100,
    cashSalesTotal: salesTotal,
    expensesTotal: expTotal,
    expectedCash: expected,
    expectedInDrawer: expected,
    withdrawAmount: Math.max(0, toWithdraw),
    countedCash: counted,
    difference,
...
export function buildCashCutTicketHtml({
  ...
  return `
    <div style="font-family:monospace;font-size:12px;line-height:1.35;padding:4px;">
      ...
      <hr style="border:none;border-top:1px dashed #000;margin:8px 0;" />
      <p>${es ? 'Fondo fijo (se queda)' : 'Float (stays)'}: ${money(cut.floatAmount || 0)}</p>
      <p>${es ? 'Ventas efectivo' : 'Cash sales'}: ${money(cut.cashSalesTotal ?? cut.expectedCash)}</p>
      <p>${es ? 'Gastos caja chica' : 'Petty cash'}: −${money(cut.expensesTotal || 0)}</p>
      <p><strong>${es ? 'Esperado en caja' : 'Expected in drawer'}:</strong> ${money(cut.expectedInDrawer ?? cut.expectedCash)}</p>
      <p><strong>${es ? 'A retirar' : 'To withdraw'}:</strong> ${money(cut.withdrawAmount ?? cut.countedCash)}</p>
      <p><strong>${es ? 'Contado en caja' : 'Counted in drawer'}:</strong> ${money(cut.countedCash)}</p>
    ...
    : (es ? '⚠ NO COINCIDE' : '⚠ MISMATCH')}</p>
      ...
      <hr style="border:none;border-top:1px dashed #000;margin:8px 0;" />
      <p style="font-weight:900;margin:0 0 4px;">${es ? 'Tickets efectivo' : 'Cash tickets'} (${cut.ticketCount || 0})</p>
      ${lines || `<p style="font-size:10px;">${es ? '(Sin ventas en efectivo)' : '(No cash sales)'}</p>`}
      ${more}
      <p style="font-weight:900;margin:8px 0 4px;">${es ? 'Gastos caja chica' : 'Petty cash'} (${cut.expenseCount || 0})</p>
      ${(cut.expenseLines || []).slice(0, 30).map((e) => (
        `<p style="font-size:10px;margin:2px 0;">−${money(e.amount)} ${escapeHtml(e.reason || '')}</p>`
      )).join('') || `<p style="font-size:10px;">${es ? '(Sin gastos)' : '(No expenses)'}</p>`}
      ${footer}
    </div>
  `;
}
