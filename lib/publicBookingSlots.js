...
function slotBlocked(timeStr, equipment, targetDate, dur, buf, dbAppointments, dbBlockedSlots, excludeAppointmentId = null) {
  ...
  const isBlocked = dbBlockedSlots.some((b) => {
    if (!blockedSlotAppliesToDate(b, targetDate)) return false;
...
export function buildDaySlots({
  ...
  if (!isClinicOpenOnDate(dbConfig, selectedDate)) return [];
  ...
  const { startMins, endMins } = getServiceScheduleBounds(service || {}, dbConfig, selectedDate);
