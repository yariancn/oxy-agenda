...
import { isShenandoah, resolveNotifyClinicDisplayName } from './clinicRegistry.js';
...
/**
 * Notify staff when a patient replies YES/NO to confirmation SMS.
 * Houston only for now — GDL just updates the agenda (no SMS to clinic phones).
 */
export async function dispatchStaffConfirmationReplyAlert({
  ...
  // GDL: reply is visible on the appointment card; do not text the clinic phone.
  if (!isShenandoah(clinicName)) {
    return { skipped: true, reason: 'gdl_agenda_only' };
  }
