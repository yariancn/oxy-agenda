...
export function buildStaffBookingAlertContent({
  ...
  const smsBody = es
    ? `${firstLine}Nueva cita (${sourceLabel}). ${patientName}. ${formattedDate} ${time}. ${equipment || ''}.${promoLine}`.trim()
    : `${firstLine}New appt (${sourceLabel}). ${patientName}. ${formattedDate} ${time}. ${equipment || ''}.${promoLine}`.trim();
