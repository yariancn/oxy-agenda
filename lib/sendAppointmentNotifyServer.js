...
async function loadPatientNotifyPrefs(clinicName, appointment) {
  ...
  if (!patientId) {
    return {
      ...
      prefers_sms_reminder: appointment?.prefers_sms_reminder !== false,
  ...
  try {
    ...
    return {
      ...
      prefers_sms_reminder: data.prefers_sms_reminder !== false,
...
export async function sendAppointmentNotifyServer({
  ...
  const { sendEmail, sendSms } = resolveNotifyChannelsForPatient(companyConfig, notifyType, {
    ...
    prefers_sms_reminder: patientPrefs.prefers_sms_reminder,
  });
