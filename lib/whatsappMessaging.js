...
export async function probeWhatsAppHealth() {
  ...
  const phoneFields = 'id,display_phone_number,verified_name,quality_rating,platform_type,code_verification_status';
  ...
  const wabaAccountId = waba && typeof waba === 'object' ? waba.id : waba;
  ...
  if (wabaAccountId) {
    for (const name of uniqueTemplates) {
      ...
      templateStatus[name] = approved
        ? { status: 'APPROVED', language: approved.language || null }
        : rows.length
          ? { status: rows[0].status, language: rows[0].language || null }
          : { status: tplRes.ok ? 'NOT_FOUND' : 'LOOKUP_FAILED', error: tplRes.data?.error?.message };
    }
  }

  const accountMode = phone.account_mode || null;
  const isTestNumber = /test number/i.test(String(phone.verified_name || ''))
    || String(phone.display_phone_number || '').replace(/\D/g, '').startsWith('1555');
  const liveOk = !isTestNumber
    && phone.code_verification_status === 'VERIFIED'
    && (accountMode === 'LIVE' || phone.platform_type === 'CLOUD_API');
  const patientTemplatesReady = ['booking', 'reschedule', 'cancel'].every(
    (type) => templateStatus[getWhatsAppTemplateName(type)]?.status === 'APPROVED',
  );

  return {
    ok: liveOk,
    readyForPatients: liveOk && patientTemplatesReady,
    isTestNumber,
    phone: {
      id: phone.id,
    ...
    patientTemplatesReady,
    hints: isTestNumber
      ? ['WHATSAPP_PHONE_NUMBER_ID apunta al número de prueba de Meta (+1 555…). Sustitúyelo por el Phone Number ID de producción en Meta Business.']
      : !liveOk
        ? ['El número no está verificado o la cuenta no está en modo LIVE. Completa registro en Meta Business Manager.']
        : [],
  };
}
