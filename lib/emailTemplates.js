...
export function mergeEmailTemplates(config = {}, locale = 'es') {
  const defaults = defaultEmailTemplates(locale);
  ...
  return {
    first: {
      subject: pick('notify_subject_first', defaults.first.subject),
      body: pick('notify_body_first', defaults.first.body),
    },
    booking: {
      subject: pick('notify_subject_booking', defaults.booking.subject),
      body: pick('notify_body_booking', defaults.booking.body),
    },
    reschedule: {
      subject: pick('notify_subject_reschedule', defaults.reschedule.subject),
      body: pick('notify_body_reschedule', defaults.reschedule.body),
    },
    cancel: {
      subject: pick('notify_subject_cancel', defaults.cancel.subject),
      body: pick('notify_body_cancel', defaults.cancel.body),
    },
    extraInfo: pick('notify_extra_info', defaults.extraInfo),
  };
}
...
export const EMAIL_PLACEHOLDER_HINT =
  '{{nombre}} {{fecha}} {{hora}} {{servicio}} {{clinica}} {{direccion}} {{telefono}} {{instrucciones}}';
