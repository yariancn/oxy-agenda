...
export function getServiceScheduleBounds(service, companyConfig = {}) {
  const start_time = normalizeTimeInput(service?.start_time) || normalizeTimeInput(companyConfig.start_time) || '07:00';
  const end_time = normalizeTimeInput(service?.end_time) || normalizeTimeInput(companyConfig.end_time) || '20:00';
  return {
    start_time,
    end_time,
