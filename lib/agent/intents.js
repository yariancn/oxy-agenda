...
const INTENT_MATCHERS = [
  {
    ...
    match(tokens, folded) {
      if (hasFuzzyToken(tokens, AUDIT_WORDS) && hasFuzzyToken(tokens, [...DESIGN_WORDS, 'sistema', 'permiso', 'riesgo'])) {
        return true;
      }
      return /auditoria/.test(folded) && /(diseno|sistema|permiso|riesgo|falla)/.test(folded);
    },
  },
  {
