...
export function extractPatientSearchQuery(message) {
  ...
  const prefixes = [
    /^busca(?:r)?\s+pacientes?\s*/i,
    /^encuentra(?:r)?\s+pacientes?\s*/i,
    /^search(?:ing)?\s+patients?\s*/i,
  ];
...
export function parseMessageParams(message, { referenceDate, clinic } = {}) {
  ...
  let fullDate = '';
  if (/\bhoy\b|\btoday\b/i.test(blob)) {
    fullDate = referenceDate || formatClinicDateIso(new Date(), clinic);
  } else if (/\bmañana\b|\bmanana\b|\btomorrow\b/i.test(blob)) {
  ...
  const patientPatterns = [
    /pacientes?\s+(.+?)(?:\s+\d{1,2}\s*[:.]|$)/i,
    /para\s+([A-ZÁÉÍÓÚÑ][a-záéíóúñ]+(?:\s+[A-ZÁÉÍÓÚÑ][a-záéíóúñ]+)+)/,
    /cita\s+(?:de|para)\s+([A-ZÁÉÍÓÚÑ][a-záéíóúñ]+(?:\s+[A-ZÁÉÍÓÚÑ][a-záéíóúñ]+)+)/i,
  ];
  ...
  let equipment = '';
  const cam = blob.match(/c[aá]mara\s*(\d+)/);
