...
import { buildGeneralHelp, isHelpQuestion, resolveHelpTopic } from './helpGuide.js';
...
export async function handleAgentMessage({
  ...
  // Prefer a matched guide: how-to questions, or HBOT clinical guides by keyword.
  const matchedTopic = resolveHelpTopic(message, { roleLevel: ctx.roleLevel, locale });
  const forceHbotGuide = matchedTopic && String(matchedTopic.topicId || '').startsWith('hbot_');
  if (forceHbotGuide || isHelpQuestion(message)) {
