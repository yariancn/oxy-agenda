...
• If new, create them when booking or under Patients`,
  ...
  {
    id: 'open_chart',
    signals: ['expediente', 'chart', 'historial', 'cartera', 'saldo', 'sesiones', 'adeudo', 'wallet'],
...
export function isHelpQuestion(message) {
  ...
  const starters = [
    /^como\s+/,
    /^como\s+(puedo|hago|se|le)\b/,
    /^donde\s+(esta|queda|puedo|veo|encuentro)\b/,
    /^que\s+hago\b/,
    /^ayuda\b/,
    /^help\b/,
    /^no\s+(lo\s+)?encuentro\b/,
    /^no\s+aparece\b/,
    /^explicame\b/,
    /^necesito\s+(saber|ayuda)\b/,
  ];
  return starters.some((p) => p.test(folded));
}
...
function scoreTopic(topic, tokens, folded) {
  ...
  if (/^como\s/.test(folded) && topic.id === 'charge_client' && hasFuzzyToken(tokens, ['cobrar', 'cobro', 'pago'])) {
    score += 5;
  }
  return score;
}
...
export function resolveHelpTopic(message, { roleLevel = ROLE_LEVEL.STAFF, locale = 'es' } = {}) {
  ...
  for (const topic of HELP_TOPICS) {
    const minLevel = topic.minLevel ?? ROLE_LEVEL.STAFF;
