...
import { HBOT_HELP_TOPICS, HBOT_SIGNAL_WORDS, isHbotGuideQuestion } from './hbotGuide.js';
import { foldAgentText, fuzzyMatchToken, hasFuzzyToken, tokenizeAgentText } from './textUnderstand.js';

const HBOT_DOMAIN_FOLDED = HBOT_SIGNAL_WORDS.map((w) => foldAgentText(w)).filter(Boolean);
const HBOT_CLINICAL_KEYS = [
  'contraindicacion',
  'contraindicaciones',
  'contraindication',
  'contraindications',
...
function messageHasHbotDomain(tokens, folded) {
  if (HBOT_DOMAIN_FOLDED.some((w) => w && folded.includes(w))) return true;
  if (hasFuzzyToken(tokens, HBOT_DOMAIN_FOLDED)) return true;
  if (hasFuzzyToken(tokens, HBOT_CLINICAL_KEYS)) return true;
  return false;
}
...
export function isHelpQuestion(message) {
  ...
  // Clinical HBOT screening questions (with or without "?").
  if (isHbotGuideQuestion(message)) return true;
  ...
  const patterns = [
    ...
    /\bhow\s+(do|can|to)\b/,
    /\b(contraindicacion|contraindicaciones|indicacion|indicaciones)\b/,
  ];
...
function topicMatches(topic, tokens, folded) {
  ...
  // HBOT guides only when the message is about OHB / clinical indication language.
  if (/^hbot_/.test(topic.id) && !messageHasHbotDomain(tokens, folded)) return false;
...
function scoreTopic(topic, tokens, folded) {
  ...
  if (/^hbot_/.test(topic.id)) {
    if (/\b(contraindic|prohibid|apto|riesgo|seguridad|pneumo|neumoto)\b/.test(folded)
      && topic.id === 'hbot_contraindications') {
      score += 10;
    }
    if (/\b(indicacion|indicaciones|sirve|trata|uhms|benefic)\b/.test(folded)
      && topic.id === 'hbot_indications') {
      score += 10;
    }
    if (/\b(cribado|screening|evalu|candidato|terapia|sesion)\b/.test(folded)
      && topic.id === 'hbot_screening') {
      score += 8;
    }
    // Prefer HBOT topics whenever hyperbaric terms appear.
    if (/\b(hiperbar|hbot|ohb|oxigenoterap|oxigenacion|camara|hyperbaric)\b/.test(folded)) {
      score += 6;
    }
  }
  return score;
...
export function buildGeneralHelp({ roleLevel = ROLE_LEVEL.STAFF, locale = 'es' } = {}) {
  const linesEs = [
    'Puedo explicarte cómo hacer cosas en OXY Agenda y cribado básico de OHB. Ejemplos:',
    '• «¿Cómo cobro a un cliente?»',
    '• «¿Cómo creo una cartera compartida?»',
    '• «¿Cómo agendo una cita?»',
    '• «No encuentro un paciente»',
    '• «¿Dónde veo el expediente?»',
    '• «Contraindicaciones hiperbárica»',
    '• «Indicaciones UHMS»',
  ...
  const linesEn = [
    'I can explain OXY Agenda how-tos and a quick HBOT screen. Examples:',
