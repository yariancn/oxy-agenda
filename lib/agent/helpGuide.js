...
2. Baja a «Paquetes, cartera compartida y cobros».
3. Cobra el paquete completo a la titular (p. ej. 10 sesiones).
4. En cartera compartida 👥:
   • Crea el grupo (o únete a uno existente)
   • Agrega a la(s) otra(s) paciente(s) como miembros
5. Solo la titular cobra; todas consumen del mismo saldo al sellar bitácora.

Ejemplo: Marisol + Ma. Eugenia con 10 sesiones → cobra 10 a la titular,
agrégalas al mismo grupo; cada una toma 5 y el saldo llega a 0.
...
2. Scroll to «Packages, shared wallet & payments».
3. Charge the full package to the owner (e.g. 10 sessions).
4. Under shared wallet 👥:
   • Create the group (or join an existing one)
   • Add the other patient(s) as members
5. Only the owner charges; everyone consumes the same balance when sealing check-in.

Example: two patients share 10 → charge 10 to the owner, add both to the group;
