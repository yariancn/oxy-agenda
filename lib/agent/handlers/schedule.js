...
import { buildEmptyScheduleHelp, buildPatientNotFoundHelp } from '../helpGuide.js';

function es(locale, esText, enText) {
  return locale === 'en' ? enText : esText;
}

function formatPatientList(patients, locale, query = '') {
  if (!patients.length) return buildPatientNotFoundHelp(query, locale);
...
function formatSchedule(appointments, locale, dateLabel) {
  if (!appointments.length) {
    return buildEmptyScheduleHelp(dateLabel, locale);
  }
...
export async function handleSearchPatient({ ctx, params, services, locale }) {
  ...
  return {
    ok: true,
    reply: `${es(locale, 'Resultados', 'Results')} (${patients.length}):\n${formatPatientList(patients, locale, q)}`,
    data: { patients },
  };
}
...
export async function handleBookAppointment({ ctx, params, services, locale }) {
  ...
  if (phoneDigits.length === 10) {
    ...
    if (ensured.is_blocked) {
      return {
        ok: false,
        error: 'validation_failed',
        message: es(
          locale,
          `Paciente bloqueado${ensured.block_reason ? `: ${ensured.block_reason}` : ''}. No se puede agendar.`,
          `Patient blocked${ensured.block_reason ? `: ${ensured.block_reason}` : ''}. Cannot book.`,
        ),
      };
    }
    ...
    const existing = (await services.listPatients({ search: patientName }))
    ...
    if (existing) {
      if (existing.is_blocked) {
        return {
          ...
          message: es(
            locale,
            `Paciente bloqueado${existing.block_reason ? `: ${existing.block_reason}` : ''}. No se puede agendar.`,
            `Patient blocked${existing.block_reason ? `: ${existing.block_reason}` : ''}. Cannot book.`,
          ),
        };
      }
