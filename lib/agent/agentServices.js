...
import { foldAgentText } from './textUnderstand.js';
...
export function createAgentServices(clinic) {
  ...
  return {
    ...
    async listPatients({ search = '', limit = null } = {}) {
      const q = String(search || '').trim();
      const foldedQ = foldAgentText(q);
      const tokens = foldedQ.split(/\s+/).filter((t) => t.length >= 1);
      const digits = q.replace(/\D/g, '');

      let rows = [];
      if (q) {
        const ilikeNeedle = tokens[0] || foldedQ;
        for (const col of ['Name', 'name']) {
          // No artificial cap — paginate until exhausted if PostgREST page size kicks in.
          let from = 0;
          const step = 1000;
          const collected = [];
          while (true) {
        ...
        // Phone search: load candidates by last digits when name query looks like a phone.
        if (!rows.length && digits.length >= 4) {
          for (const col of ['Phone', 'phone']) {
            const { data, error } = await supabase
              .from('patients')
              .select('*')
              .ilike(col, `%${digits.slice(-10)}%`);
      ...
      rows = filterRowsByClinic(rows, clinicId);
      const needle = normalizeStr(q);
      const filtered = rows.filter((p) => {
        const name = normalizeStr(p.Name || p.name || '');
        ...
        if (needle) return name.includes(needle);
        return true;
      });
      const mapped = filtered.map(normalizePatientRow);
      if (limit != null && Number(limit) > 0) return mapped.slice(0, Number(limit));
      return mapped;
    },
