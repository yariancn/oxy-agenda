...
export function createAgentServices(clinic) {
  ...
  return {
    ...
    async listPatients({ search = '', limit = 8 } = {}) {
      ...
      const tokens = foldedQ.split(/\s+/).filter((t) => t.length >= 2);
      ...
      if (q) {
        ...
        for (const col of ['Name', 'name']) {
          const { data, error } = await supabase.from('patients').select('*').ilike(col, `%${ilikeNeedle}%`).limit(120);
        ...
        if (!rows.length && tokens.length > 1) {
          for (const col of ['Name', 'name']) {
            const { data, error } = await supabase.from('patients').select('*').ilike(col, `%${tokens.join('%')}%`).limit(120);
            if (!error && data?.length) {
              rows = data;
              break;
            }
