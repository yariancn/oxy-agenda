    ...
    async listPatients({ search = '', limit = 8 } = {}) {
      ...
      if (q) {
        for (const col of ['Name', 'name']) {
          const { data, error } = await supabase.from('patients').select('*').ilike(col, `%${q}%`).limit(80);
        ...
        if (!rows.length && tokens.length > 1) {
          for (const col of ['Name', 'name']) {
            const { data, error } = await supabase.from('patients').select('*').ilike(col, `%${tokens[0]}%`).limit(120);
            if (!error && data?.length) {
              rows = data;
              break;
            }
