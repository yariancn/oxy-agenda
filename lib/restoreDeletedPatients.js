...
export async function restoreDeletedPatients(supabase, {
  ...
  const restoreOne = async ({
    ...
    const relatedApps = (appointments || []).filter(
    ...
    const agg = aggregateFromAppointments(relatedApps);
    const phoneDisplay = agg.phone
      ? (agg.phone.startsWith('33') || agg.phone.startsWith('55') ? `+52 ${agg.phone}` : agg.phone)
      : (relatedApps[0]?.phone || relatedApps[0]?.Phone || '');
  ...
  // 2) Restore charts visible on agenda but missing from Pacientes (by appointment name).
  ...
  // 3) Ensure single Lucia Torres (remove extras if any reappeared).
