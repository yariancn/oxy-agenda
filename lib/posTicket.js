...
export function buildPosTicketHtml({
  ...
  const clinic = clinicName === 'Shenandoah' ? 'Shenandoah' : 'Guadalajara';
  const currency = clinic === 'Shenandoah' ? 'USD' : 'MXN';
  const legal = LEGAL_FOOTER[clinic][locale === 'en' ? 'en' : 'es'];
  const links = getLegalLinks(clinic);
  const clinicDisplay = formatClinicField(companyConfig.name) || (clinic === 'Shenandoah' ? 'OXYHYPERBARIC' : 'OXYGENGDL');
