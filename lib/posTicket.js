...
import { formatSalePaymentMethod, getPaymentSplits } from './paymentMethod.js';
...
export function buildPosTicketHtml({
  ...
  return `
    <div style="width:52mm;max-width:52mm;font-family:monospace;font-size:10px;text-transform:uppercase;color:#000;">
      ...
      ${(() => {
        const paidLocale = locale === 'en' ? 'en' : 'es';
        const splits = getPaymentSplits(receipt, paidLocale);
        if (splits.length >= 2) {
          return `<p style="margin:0;">${labels.receiptPaidWith || 'PAGADO CON:'}</p>${
            splits.map((row) => (
              `<p style="margin:0 0 0 4px;font-size:10px;">• ${escapeHtml(row.label)}: ${money(row.amount, currency)}</p>`
            )).join('')
          }`;
        }
        return `<p style="margin:0;">${labels.receiptPaidWith || 'PAGADO CON:'} ${escapeHtml(formatSalePaymentMethod(receipt, paidLocale) || receipt.paymentMethod || '')}</p>`;
      })()}
...
export function buildPosTicketSmsText({
  ...
  // Mismos datos que el ticket impreso (buildPosTicketHtml), en texto plano.
  const lines = [
    clinicDisplay,
    address,
    phone ? `${labels.receiptPhone || 'Tel:'} ${phone}` : '',
    ...
    receipt.serviceName ? String(receipt.serviceName).toUpperCase() : '',
    `${labels.receiptSessions || (es ? 'Sesiones:' : 'Sessions:')} ${sessions} × ${money(unitPrice, currency)}`,
    `${labels.receiptSubtotal || 'SUBTOTAL:'} ${money(subtotal, currency)}`,
    `${labels.receiptTotal || 'TOTAL:'} ${money(total, currency)}`,
    `${labels.receiptPaidWith || (es ? 'PAGADO CON:' : 'PAID WITH:')} ${formatSalePaymentMethod(receipt, locale) || receipt.paymentMethod || ''}`,
    receipt.operator ? `${labels.receiptServedBy || (es ? 'Le atendió:' : 'Served by:')} ${receipt.operator}` : '',
    receipt.ticketNotes ? `${labels.receiptNotes || 'NOTAS:'} ${receipt.ticketNotes}` : '',
    thanks,
  ].filter(Boolean);
