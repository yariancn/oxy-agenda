...
export function getAvailableWalletEntry(wallets = {}, equipment, servicePrice = null) {
  ...
  if (sharedPool) {
    // Only same price tier or chamber-named keys — never steal another price_* pool.
    if (priceKey && (Number(next[priceKey]) || 0) > 0) {
      return { key: priceKey, balance: Number(next[priceKey]) };
    }
    for (const [key, balance] of Object.entries(next)) {
  ...
  // No last-resort cross-service consume on live seal — that caused wrong-package deductions.
  return null;
}

/** Consume any positive wallet key (reconcile / orphan cleanup only — not live seals). */
export function consumeAnyWalletBalance(wallets = {}) {
  const next = { ...wallets };
  let bestKey = null;
  let bestBal = 0;
  for (const [key, balance] of Object.entries(next)) {
  ...
  if (!bestKey) return { wallets: next, deducted: false, walletKey: null };
  next[bestKey] = Math.max(0, bestBal - 1);
  return { wallets: next, deducted: true, walletKey: bestKey };
}
...
export function reversePurchaseSessions(wallets = {}, adeudo = 0, tx = {}, packageHistory = null) {
  ...
  const nextAdeudo = Math.max(0, (Number(adeudo) || 0) + debtCleared + Math.max(0, remaining));

  return { wallets: nextWallets, adeudo: nextAdeudo, usedAlready: Math.max(0, remaining) };
}
 ...
 * Prefer `chargedVia` from the visit (`wallet` | `adeudo`) so undo doesn't clear unrelated debt.
 */
export function reverseNoShowWalletImpact(wallets = {}, adeudo = 0, {
  ...
  const via = chargedVia === 'wallet' || chargedVia === 'adeudo'
    ? chargedVia
    : (currentAdeudo > 0 ? 'adeudo' : 'wallet');

  if (via === 'adeudo' && currentAdeudo > 0) {
    return {
...
export function parseAppointmentChargeVia(notes = '') {
  const m = String(notes || '').match(/⟦oxy:(wallet|adeudo)⟧/);
  return m ? m[1] : null;
}

export function appendAppointmentChargeVia(notes = '', chargedVia) {
  const cleaned = String(notes || '').replace(/\s*⟦oxy:(wallet|adeudo)⟧/g, '').trim();
  if (chargedVia !== 'wallet' && chargedVia !== 'adeudo') return cleaned;
  return cleaned ? `${cleaned}\n⟦oxy:${chargedVia}⟧` : `⟦oxy:${chargedVia}⟧`;
}

const SEAL_CHARGE_SETTLED_MARK = '⟦oxy:cobrado⟧';

/** True cuando el sello anterior ya cobró la sesión: el re-sello no debe volver a cobrar. */
export function hasSettledSealCharge(notes = '') {
  return String(notes || '').includes(SEAL_CHARGE_SETTLED_MARK);
}

export function markSettledSealCharge(notes = '') {
  const cleaned = String(notes || '').trim();
  if (hasSettledSealCharge(cleaned)) return cleaned;
  return cleaned ? `${cleaned}\n${SEAL_CHARGE_SETTLED_MARK}` : SEAL_CHARGE_SETTLED_MARK;
}

export function clearSettledSealCharge(notes = '') {
  return String(notes || '').replace(/\s*⟦oxy:cobrado⟧/g, '').trim();
}
...
export function reconcileAdeudoAgainstWallet(wallets = {}, adeudo = 0, packageHistory = []) {
  ...
  while (nextAdeudo > 0 && sumWalletBalance(nextWallets) > 0) {
    const consumed = consumeAnyWalletBalance(nextWallets);
 ...
 *
 * Prefer `chargedFromAppointments` (count of Finalizado/No Asistió) over the
 * stored historico_sesiones counter when provided — that counter can lag.
 *
...
export function reconcilePatientWalletState({
  ...
  const storedHistorico = Math.max(0, Number(historicoSesiones) || 0);
  const historico = chargedFromAppointments != null
    ? Math.max(0, Number(chargedFromAppointments) || 0)
    : storedHistorico;
  let trimmed = 0;

  // Excess paid sessions left after all charged takes:
  // (historico + wallet) - (purchased + adeudo)
  // Skip silent trim when purchase history is missing — wiping orphan paid wallets is worse.
  let excess = (historico + sumWalletBalance(nextWallets)) - (purchased + nextAdeudo);
  if (purchased <= 0 && sumWalletBalance(nextWallets) > 0) {
    excess = 0;
  }
  ...
  return {
    wallets: nextWallets,
    adeudo: nextAdeudo,
    historicoSesiones: historico,
    cleared: step1.cleared,
    trimmed,
    purchased,
    historico,
    pending: sumWalletBalance(nextWallets),
    historicoSynced: historico !== storedHistorico,
    changed: step1.changed || trimmed > 0 || historico !== storedHistorico,
  };
}
