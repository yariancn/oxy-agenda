...
/** Solo servicios activos de equipo — valoración va en franja compacta, no en columna. */
export function buildCalendarEquipmentColumns(services = []) {
  const all = activeServiceNames(services);
  const assessmentService = all.find((name) => /valoraci[oó]n/i.test(name)) || null;
  const columns = all.filter((name) => !/valoraci[oó]n/i.test(name));
  return {
    columns,
    assessmentService,
    orphans: new Set(),
