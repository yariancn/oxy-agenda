...
function buildShenandoahSmsFooter({ clinicPhone, locale = 'en' }) {
  const phone = formatUsPhoneDisplay(clinicPhone || '7135913379');
  if (locale === 'en') {
    return `Please contact us at ${phone} for any questions.`;
  }
  return `Para cualquier duda sobre tu cita, comunícate al ${phone}.`;
}
...
export function buildNotifyContent({
  ...
  const smsParts = [
    ...
    trimmedInstructions
      ? (sessionInstructionsUrl
          ? (es
              ? `IMPORTANTE: lee las indicaciones de tu sesión aquí: ${sessionInstructionsUrl}`
              : `IMPORTANT: read your session instructions here: ${sessionInstructionsUrl}`)
    ...
    isGdlCluster(clinicName)
      ...
      : isShenandoah(clinicName)
        ? buildShenandoahSmsFooter({ clinicPhone, locale: effectiveLocale })
        : '',
  ].filter(Boolean);
