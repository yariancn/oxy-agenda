...
export function formatLocationForSms({ address = '', mapsUrl = '', locale = 'es' } = {}) {
  ...
  const openLabel = es ? 'Ver mapa' : 'Map';

  if (location.address && location.mapsUrl) {
    return `${label}: ${location.address} ${openLabel}: ${location.mapsUrl}`;
  }
  if (location.mapsUrl) {
    return `${label}: ${location.mapsUrl}`;
  }
  return `${label}: ${location.address} ${buildGoogleMapsUrl(location.address)}`.trim();
}
