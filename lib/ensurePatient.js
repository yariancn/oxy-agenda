...
/**
 * Staff choice when phone already belongs to someone else.
 * @returns {'use_existing' | 'create_new' | 'abort'}
 */
export function chooseDuplicatePhoneAction({
  existingName,
  typedName,
  locale = 'es',
} = {}) {
  ...
  const useExisting = typeof window !== 'undefined' && window.confirm(
    es
      ? `Ya tenemos a «${existing}» dado de alta con este número.\n\nAceptar = usar ese paciente\nCancelar = ver opción de dar de alta a otro`
      : `«${existing}» is already registered with this phone.\n\nOK = use that patient\nCancel = see option to register someone else`,
  );
  if (useExisting) return 'use_existing';

  const createNew = typeof window !== 'undefined' && window.confirm(
    es
      ? `¿Dar de alta a «${typed}» como paciente NUEVO con el mismo número?\n\nAceptar = crear otro paciente\nCancelar = no guardar`
      : `Register «${typed}» as a NEW patient with the same phone?\n\nOK = create another patient\nCancel = don't save`,
  );
