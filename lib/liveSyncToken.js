...
export async function computeLiveSyncToken({ supabase, clinicName }) {
  ...
  const [appointments, blockedSlots, servicesRes, configRes] = await Promise.all([
    queryWithClinicFallback(supabase, 'appointments', clinicId, (q) =>
      q.select('id, time, full_date, equipment, check_in_status, outside_normal_hours, is_extended_block, patient, duration, buffer, day, clinic, is_new_patient, confirmation_status, confirmation_sent_at, confirmation_replied_at, confirmation_reply')
        .gte('full_date', from)
        .lte('full_date', to)
