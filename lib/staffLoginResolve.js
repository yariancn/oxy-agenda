...
import { findRoleLevelInDb, inferRoleLevelFromName, resolveLoggedInRoleLevel } from './staffRoleLevel.js';
...
async function fetchGdlMasterPin(supabaseGdl) {
  const res = await supabaseGdl
    ...
    .limit(1)
    .maybeSingle();
  if (res.data?.master_pin) return String(res.data.master_pin).trim();
...
async function fetchRoleLevel(supabase, roleName) {
  if (!roleName || !supabase) return 3;
  const normalizedRole = String(roleName).trim();
  const { data: rows } = await supabase
    .from('user_roles')
    .select('name, level');
  if (rows?.length) {
    return resolveLoggedInRoleLevel(rows, normalizedRole);
  }
  ...
  const { data } = await supabase
    ...
    .maybeSingle();
  if (data?.level != null && Number(data.level) > 0) return Number(data.level);
  return resolveLoggedInRoleLevel([], normalizedRole);
}
...
async function resolveBreakGlassPin(pin, supabaseGdl, supabaseShenandoah) {
  ...
  if (supremePin && normalizedPin === supremePin) {
    return {
      user: {
        id: 'admin',
  ...
  if (
    (gdlMaster && normalizedPin === gdlMaster)
