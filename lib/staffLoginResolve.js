import { ALL_CLINICS, gdlStaffClinicAccess, CLINIC_OXYGENDGL, CLINIC_OXYGENDGL2, CLINIC_SHENANDOAH } from './clinicRegistry.js';
...
async function fetchRoleLevel(supabase, roleName) {
  ...
  if (/administrador\s+maestro|super\s+administrador/i.test(normalizedRole)) return 1;
  return 99;
}
...
async function buildUserFromStaff(gdlStaff, txStaff, supabaseGdl, supabaseShenandoah) {
  ...
  const roleSupabase = gdlStaff ? supabaseGdl : supabaseShenandoah;
  const roleLevel = await fetchRoleLevel(roleSupabase, primary.role);
