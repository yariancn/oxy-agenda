...
export const PUBLIC_BOOKING_COPY = {
  es: {
    ...
    patientBlocked: '⚠️ Este número no puede agendar en línea. Contacta a la clínica.',
  ...
  en: {
    ...
    patientBlocked: '⚠️ This phone number cannot book online. Please contact the clinic.',
...
const STAFF = {
  es: {
    ...
    loginTrustedPinHint: 'Dispositivo recordado · confirma tu NIP (red distinta)',
    ...
    time: 'Hora',
    zoom: 'Zoom',
    filterEquipment: 'Filtrar',
    allEquipment: 'Todos',
    showAllEquipment: 'Ver todos los equipos',
    clearEquipmentFilter: 'Quitar filtro de equipo',
    ...
    ariaScreenshotCapture: 'Agendar desde captura de pantalla',
    ...
    dbErrorUnauthorized: 'Sesión expirada — vuelve a entrar con tu NIP. Si estabas sellando bitácora, la firma se guarda en el dispositivo para reintentar.',
    ...
    symbolLegend: {
      ...
      patientBlocked: 'Paciente bloqueado',
  ...
  en: {
    ...
    loginTrustedPinHint: 'Remembered device · confirm PIN (different network)',
    ...
    clearEquipmentFilter: 'Clear equipment filter',
    ...
    ariaScreenshotCapture: 'Book from screenshot',
    ...
    dbErrorRetry: 'Retry',
    dbErrorUnauthorized: 'Session expired — sign in again with your PIN. If you were sealing attendance, the signature is kept on this device for retry.',
    ...
    symbolLegend: {
      ...
      patientBlocked: 'Patient blocked',
...
const ALERTS = {
  es: {
    ...
    patientBlocked: '🚫 Paciente bloqueado. No se pueden agendar citas ni servicios. Puedes consultar su expediente y desbloquearlo ahí si corresponde.',
    patientBlockedShort: '🚫 Paciente bloqueado. No se puede programar.',
    overlap: '🔒 Empalme de horario.',
    pastSchedule: '🔒 No puedes agendar en días anteriores. El mismo día sí, aunque la hora ya haya pasado.',
    pastMove: '🔒 No puedes reubicar citas a días anteriores sin autorización.',
    pastMoveCodePrompt: 'Vas a mover la cita a un día anterior.\nIngresa el código de autorización (0000):',
    pastMoveCodeTitle: 'Autorizar movimiento a día anterior',
    pastMoveCodeLabel: 'Código (0000)',
    pastMoveCodeConfirm: 'Autorizar y continuar',
    pastMoveCodeWrong: 'Código incorrecto. La cita no se movió a un día anterior.',
    pastMoveOverrideHint: 'Autorizado con código — destino en día anterior',
    unsealTitle: 'Quitar sello para volver a firmar',
    unsealPrompt: 'Se borrará la firma y la bitácora quedará abierta para firmar de nuevo.\nNo se vuelve a cobrar la sesión.\nIngresa tu NIP para autorizar:',
    unsealPinLabel: 'Tu NIP',
    unsealConfirm: 'Quitar sello',
    unsealPinWrong: 'NIP incorrecto. El sello no se quitó.',
    unsealLocked: (mins) => `Demasiados intentos con NIP. Espera ${mins} minuto(s).`,
    unsealSessionExpired: 'Tu sesión expiró. Vuelve a entrar con tu NIP e inténtalo otra vez.',
    unsealError: 'No se pudo quitar el sello. Intenta de nuevo.',
    unsealOk: 'Sello retirado. Abre Bitácora para firmar de nuevo.',
    unsealAuditAction: 'SELLO RETIRADO',
    unsealAuditDetail: (name) => `Sello y firma eliminados por ${name} para volver a firmar. Sin nuevo cobro de sesión.`,
    resealNoChargeAudit: 'Re-sello: la sesión ya se había cobrado, no se cobró de nuevo.',
    phoneRequired: 'Ingresa un teléfono de 10 dígitos para crear el expediente.',
    mustPickPatientFromList: 'Hay un expediente con ese nombre. Elige al paciente de la lista (clic en el resultado) para usar sus datos correctos. No escribas el nombre a mano si ya existe.',
    patientFileError: (m) => `Error con expediente: ${m}`,
    missingData: 'Faltan datos.',
    missingAppointmentFields: (fields) => `Faltan: ${fields.join(', ')}.`,
    ...
    statusUpdateError: 'Error al actualizar el estatus.',
    statusLockedSealed: 'Esta cita ya está sellada. No se puede cambiar el estatus.',
    ...
    pastScheduleAppt: '🔒 No puedes agendar citas en días anteriores. Hoy sí puedes, aunque la hora ya haya pasado.',
    pastMoveClickHint: 'Para mover a un día anterior: Reprogramar → elige el horario → código 0000. El mismo día no pide código.',
    bitacoraFutureDay: '🔒 No se puede sellar una cita de un día futuro. Puedes marcar falta, no show o cancelar. Las citas de hoy o días pasados sí se pueden firmar.',
    ...
    cloneDetected: 'El paciente ya existe (mismo nombre con ese teléfono o correo). Usa el expediente existente.',
    exactPatientDuplicate: 'No se puede duplicar: ya hay un paciente con el mismo nombre y el mismo teléfono o correo.',
    sharedContactWarn: 'Ya hay un paciente con ese teléfono o correo (otro nombre). Revisa antes de guardar.',
    ...
    patientBlockedMove: '🚫 Paciente bloqueado. No se pueden reubicar ni alterar sus citas.',
    ...
    noShowAssessmentConfirm: 'Marcar NO ASISTIÓ en valoración: no afecta cartera ni adeudo. ¿Continuar?',
    undoNoShowConfirm: '¿Restaurar la sesión descontada por este no-show? El estatus NO ASISTIÓ se mantiene; solo se deshace el cobro (adeudo −1 o +1 sesión a cartera).',
    undoNoShowOk: 'Sesión restaurada. El no-show sigue registrado; el paciente no pierde esa sesión pagada.',
    excuseNoShowConfirm: '¿Pasar a FALTA JUSTIFICADA y devolver la sesión descontada? (No se cobra del paquete.)',
    excuseNoShowOk: 'Pasó a falta justificada y se restauró la sesión.',
    promoterNoShowSent: 'Correo de no-show enviado al promotor.',
    promoterNoShowNoEmail: 'No-show guardado. El promotor no tiene correo en Admin → Promotores (no se envió aviso).',
    promoterNoShowSkipped: 'No-show guardado. No hay promotor asignado o el aviso no aplica.',
    promoterNoShowError: (msg) => `No-show guardado, pero falló el correo al promotor: ${msg}`,
  ...
  en: {
    ...
    patientBlocked: '🚫 Patient blocked. Appointments cannot be booked. You can open the chart and unblock there if needed.',
    patientBlockedShort: '🚫 Patient blocked. Cannot schedule.',
    overlap: '🔒 Time slot conflict.',
    pastSchedule: '🔒 Cannot book on previous days. Same-day earlier hours are allowed.',
    pastMove: '🔒 Cannot move appointments to previous days without authorization.',
    pastMoveCodePrompt: 'You are moving this appointment to a previous day.\nEnter the authorization code (0000):',
    pastMoveCodeTitle: 'Authorize move to a previous day',
    pastMoveCodeLabel: 'Code (0000)',
    pastMoveCodeConfirm: 'Authorize and continue',
    pastMoveCodeWrong: 'Incorrect code. The appointment was not moved to a previous day.',
    pastMoveOverrideHint: 'Authorized with code — destination is a previous day',
    unsealTitle: 'Remove seal to sign again',
    unsealPrompt: 'The signature will be deleted and the attendance log reopens for a new signature.\nThe session is not charged again.\nEnter your PIN to authorize:',
    unsealPinLabel: 'Your PIN',
    unsealConfirm: 'Remove seal',
    unsealPinWrong: 'Incorrect PIN. The seal was not removed.',
    unsealLocked: (mins) => `Too many PIN attempts. Wait ${mins} minute(s).`,
    unsealSessionExpired: 'Your session expired. Sign in with your PIN and try again.',
    unsealError: 'Could not remove the seal. Please try again.',
    unsealOk: 'Seal removed. Open Attendance to sign again.',
    unsealAuditAction: 'SEAL REMOVED',
    unsealAuditDetail: (name) => `Seal and signature removed by ${name} to sign again. No new session charge.`,
    resealNoChargeAudit: 'Re-seal: session was already charged, not charged again.',
    phoneRequired: 'Enter a 10-digit phone number to create the chart.',
    mustPickPatientFromList: 'A chart already exists with that name. Pick the client from the list (click the result) so we use the correct data. Do not type the name freehand if they already exist.',
    patientFileError: (m) => `Chart error: ${m}`,
    missingData: 'Missing required fields.',
    missingAppointmentFields: (fields) => `Missing: ${fields.join(', ')}.`,
    ...
    statusLockedSealed: 'This appointment is already sealed. Status cannot be changed.',
    ...
    pastScheduleAppt: '🔒 Cannot book appointments on previous days. Today is allowed even for earlier hours.',
    pastMoveClickHint: 'To move to a previous day: Reschedule → pick the time → code 0000. Same-day moves need no code.',
    bitacoraFutureDay: '🔒 Cannot seal an appointment on a future day. You can mark excused, no-show, or cancel. Today and past days can still be signed.',
    ...
    cloneDetected: 'This client already exists (same name with that phone or email). Use the existing chart.',
    exactPatientDuplicate: 'Cannot duplicate: a client with the same name and the same phone or email already exists.',
    sharedContactWarn: 'Another client already uses that phone or email (different name). Review before saving.',
    ...
    patientBlockedMove: '🚫 Patient blocked. Appointments cannot be moved or changed.',
    ...
    noShowAssessmentConfirm: 'Mark NO-SHOW for assessment: wallet and debt are not affected. Continue?',
    undoNoShowConfirm: 'Restore the session taken by this no-show? Status stays NO-SHOW; only the charge is reversed (debt −1 or +1 session to wallet).',
    undoNoShowOk: 'Session restored. No-show stays on record; the client does not lose that paid session.',
    excuseNoShowConfirm: 'Mark as EXCUSED ABSENCE and restore the deducted session? (Does not consume the package.)',
    excuseNoShowOk: 'Marked excused and session restored.',
    promoterNoShowSent: 'No-show email sent to the promoter.',
    promoterNoShowNoEmail: 'No-show saved. Promoter has no email in Admin → Promoters (notice not sent).',
    promoterNoShowSkipped: 'No-show saved. No promoter assigned or notice not applicable.',
    promoterNoShowError: (msg) => `No-show saved, but promoter email failed: ${msg}`,
    bitacoraNoBalanceConfirm: (nextDebt) =>
...
const STATUS_LABELS = {
  es: {
    ...
    Devuelto: 'Devuelto',
    Cancelado: 'Cancelado',
    'Pendiente cancelación': 'Pendiente cancelación',
  },
  en: {
    Agendado: 'Scheduled',
    'Llegó': 'Arrived',
    ...
    Devuelto: 'Refunded',
    Cancelado: 'Cancelled',
    'Pendiente cancelación': 'Cancel pending',
  },
};
