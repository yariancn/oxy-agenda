...
import {
  patientPrefersSmsReminder,
  patientPrefersSmsScheduling,
...
export async function loadPatientPreferSms(supabase, { patientId, phone }) {
  if (patientId) {
    ...
    if (pat) {
      return {
        ...
        prefersSmsReminder: pat.prefers_sms_reminder,
    ...
    for (const col of ['Phone', 'phone']) {
      ...
      if (match) {
        return {
          ...
          prefersSmsReminder: match.prefers_sms_reminder,
  ...
  return {
    phone: '',
    prefersSms: undefined,
    prefersSmsReminder: undefined,
    patientName: '',
    found: false,
  };
}
...
export async function runStaffPatientSms({
  ...
  const reminderLike = preset === 'reminder' || preset === 'waiting';
  const smsAllowed = reminderLike
    ? patientPrefersSmsReminder(fromPatient.prefersSmsReminder)
    : patientPrefersSmsScheduling(fromPatient.prefersSms);
