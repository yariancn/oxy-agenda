...
const SELECT_MIN_LEVEL = {
  ...
  audit_logs: MANAGER,
  petty_cash_expenses: STAFF,
  cash_drawer_events: STAFF,
};
...
const MUTATION_MIN_LEVEL = {
  ...
  petty_cash_expenses: STAFF, // anyone registers petty cash
  cash_drawer_events: STAFF, // arqueo + retiro
  services: MANAGER,
  protocols: MANAGER,
  promoters: MANAGER,
...
function roleLevelOf(user) {
  ...
  // Authenticated staff cookie with legacy/unresolved 99 → treat as STAFF so agenda works.
  if (user?.id || user?.email) return STAFF;
...
export function assertStaffDbPermission(user, { table, action = 'select', data = null } = {}) {
  ...
  // Hard delete of patient charts: managers+ only (UI uses /api/staff/delete-patient).
  if (table === 'patients' && act === 'delete' && level > MANAGER) {
    const err = new Error('Insufficient level to delete patients.');
    err.status = 403;
    throw err;
  }
...
export function sanitizeStaffDbSelectData(user, table, data, select = '*') {
  ...
  // Drop signature blobs from broad appointment lists (cellular + cold start).
  // Keep them only when the client explicitly selected the signature column.
  if (table === 'appointments') {
    const selectText = String(select || '*');
    const wantsSignature = /(^|[, ])signature([, ]|$)/i.test(selectText);
    if (!wantsSignature) {
      const mapRow = (row) => {
