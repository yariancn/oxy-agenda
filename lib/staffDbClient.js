class StaffQueryBuilder {
  ...
  eq(col, val) {
    if (val == null || val === '') {
      throw new Error(`Missing filter value for ${col}. Recarga la pantalla e intenta de nuevo.`);
    }
  ...
  async execute() {
    ...
    if (!response.ok) {
      const message = body.error || 'Database request failed';
      return {
        data: null,
        error: {
          message,
    ...
    // Insert/update with .select().single() must return one row object.
    // The staff API returns an array unless we unwrap (real Supabase .single() does this).
    let data = body.data ?? null;
    if (
      (this.action === 'insert' || this.action === 'update')
      && (this.singleValue || this.maybeSingleValue)
      && Array.isArray(data)
    ) {
      data = data[0] ?? null;
      if (this.singleValue && data == null) {
        return {
          data: null,
          error: { message: 'No rows returned from insert/update.' },
