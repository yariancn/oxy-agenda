...
export function explainConfirmationState({
  appointment,
  allAppointments = [],
  companyConfig = {},
  clinicName,
  ...
  if (status === CONFIRMATION_STATUS.CONFIRMED) {
    return { applicable: true, sent: true, replied: true, status, summaryEs: 'Paciente confirmó por SMS.', summaryEn: 'Patient confirmed via SMS.' };
  }
  if (status === CONFIRMATION_STATUS.DECLINED) {
    return { applicable: true, sent: true, replied: true, status, summaryEs: 'Paciente respondió NO (cita cancelada).', summaryEn: 'Patient replied NO (appointment cancelled).' };
  }
  if (status === CONFIRMATION_STATUS.NO_RESPONSE) {
    return { applicable: true, sent: true, replied: false, status, summaryEs: 'SMS enviado; sin respuesta a tiempo (probable falta).', summaryEn: 'SMS sent; no reply in time (likely no-show).' };
  }
  if (status === CONFIRMATION_STATUS.PENDING) {
    return { applicable: true, sent: true, replied: false, status, summaryEs: 'SMS enviado; esperando SI/NO.', summaryEn: 'SMS sent; waiting for YES/NO.' };
  }

  if (companyConfig.confirmation_sms_enabled !== true) {
    return { applicable: true, sent: false, status, summaryEs: 'Confirmación desactivada en Admin → Mensajes (Houston).', summaryEn: 'Confirmation disabled in Admin → Messages (Houston).' };
  }
  if (!isTwilioConfigured()) {
    return { applicable: true, sent: false, status, summaryEs: 'Twilio no configurado en el servidor.', summaryEn: 'Twilio not configured on server.' };
  }
  if (!String(appointment?.phone || '').trim()) {
    return { applicable: true, sent: false, status, summaryEs: 'La cita no tiene teléfono.', summaryEn: 'Appointment has no phone number.' };
  }
  if (!isFirstSessionAppointment({
    ...
    return { applicable: true, sent: false, status, summaryEs: 'No es primera sesión en la clínica (ya tiene citas previas).', summaryEn: 'Not a first session at this clinic (prior appointments exist).' };
  }
  if (!ACTIVE_STATUSES.has(appointment?.check_in_status || 'Agendado')) {
    return { applicable: true, sent: false, status, summaryEs: `Estatus «${appointment?.check_in_status}» — no aplica envío.`, summaryEn: `Status «${appointment?.check_in_status}» — send not applicable.` };
  }
  ...
  if (!startMs) {
    return { applicable: true, sent: false, status, summaryEs: 'Fecha u hora inválida en la cita.', summaryEn: 'Invalid date or time on appointment.' };
  }
  const sendAtMs = startMs - hoursBefore * 60 * 60 * 1000;
  ...
  if (now < sendAtMs) {
    ...
    return { applicable: true, sent: false, status, summaryEs: `Aún no toca enviar (programado ~${when}).`, summaryEn: `Not time to send yet (scheduled ~${when}).` };
  }
  if (now > tooLateMs) {
    return { applicable: true, sent: false, status, summaryEs: 'Ventana de envío ya pasó (faltan menos de 30 min).', summaryEn: 'Send window passed (less than 30 min to appointment).' };
  }
  return { applicable: true, sent: false, status, summaryEs: 'Debería enviarse en la próxima revisión automática.', summaryEn: 'Should send on the next automatic check.' };
}
