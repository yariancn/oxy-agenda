...
export async function refreshStaffSessionUser(user) {
  ...
  const mergedProfiles = {
    ...(user.clinicProfiles || {}),
    ...(refreshed.clinicProfiles || {}),
  };
  const mergedAllowed = [
    ...(user.allowedClinics || []),
    ...(refreshed.allowedClinics || []),
    ...Object.keys(mergedProfiles),
  ];
  const accessLevel = Math.min(
    Number(user.accessLevel) || 99,
    Number(refreshed.accessLevel) || 99,
  );
  ...
  return normalizeStaffSessionUser({
    ...
    clinicProfiles: mergedProfiles,
    allowedClinics: mergedAllowed,
    accessLevel,
...
export async function resolveStaffAutoLoginServer({ request }) {
  ...
  return { user, device: trustedDevice };
}
